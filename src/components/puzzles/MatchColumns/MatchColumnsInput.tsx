...
export default function MatchColumnsInput() {
  const { setMatchColumnsResult, setMatchColumnsTitle, setLoading } = usePuzzleStore();
  ...
  const handleGenerate = () => {
    ...
    if (lines.length === 0) {
      toast.warning("Ingresa al menos un par palabra-definicion");
      return;
    }
    ...
    if (items.length < 2) {
      toast.warning("Necesitas al menos 2 pares validos (minimo 2 letras y con definicion)");
      return;
    }

    setMatchColumnsTitle(title);
    setLoading(true);
