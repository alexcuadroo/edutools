...
export default function HangmanInput() {
  const { setHangmanResult, setHangmanTitle, setLoading } = usePuzzleStore();
  ...
  const handleGenerate = () => {
    ...
    if (items.length === 0) {
      toast.warning("Ninguna palabra valida (minimo 2 letras)");
      return;
    }

    setHangmanTitle(title);
    setLoading(true);

    setTimeout(() => {
      try {
        const result = hangmanGenerator.generate({ words: items });
        setHangmanResult(result.grid);
