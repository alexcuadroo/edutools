...
export default function PlayRoscoPage() {
  ...
  return <div className="py-4"><div className="mb-4 flex flex-wrap items-center justify-between gap-2"><Link to="/" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700"><ArrowLeft className="h-4 w-4" /> Volver al inicio</Link>{live.confirmed && <StudentAliasBadge alias={live.alias} />}</div><RoscoGame entries={data.entries} durationSeconds={data.durationSeconds} title={data.title} attemptCount={count} onAttemptIncrement={increment} onProgress={setProgress} />{!live.confirmed && <StudentIdentityModal alias={live.alias} onConfirm={live.confirm} />}</div>;
}
